...
df.probSeqLastPoint
bigPlot <- probLinesPlot + geom_text (data = df.probSeqLastPoint, aes (x =step+200, y =evalScore, label = cage), size=6, face="bold")
                           
df.probSeqSmall <- df.probSeq [which (df.probSeq$step > 6000), ]
smallPlot <- ggplot(df.probSeqSmall, aes(x=step, y=evalScore, group=cage)) + 
