...
# ggsave (gAllByWeek_grid, file=paste(home, "/2017_phecomp_marta/figures/", "circadian_day_night_ts_by_genotype_counts.png", sep=""))
# ggsave (gAllByWeek_grid, file=paste(home, "/2017_phecomp_marta/figures/", "circadian_day_night_ts_by_genotype_counts.tiff", sep=""), 
#         width=12, height=7, dpi=400)
...
png(paste(home, "/2017_phecomp_marta/figures/", "circadian_day_night_dyrk_by_genotype_simple_counts.png", sep=""), width=1000, height=800 )
...
png(paste(home, "/2017_phecomp_marta/figures/", "circadian_day_night_trisomics_and_dyrk_by_genotype_simple_counts2.png", sep=""), width=1000, height=400 )
...
######################
## Control, trisomics and dyrk1a
######################
######################
## Join Free choice trisomic with dyrk AND ALSO DE CONTROL ANIMALS
tbl_control_wt$genotype <- "wt control"
tbl_control_ts$genotype <- "trisomic control"
tbl_all <- rbind(tbl_control_wt, tbl_control_ts,
                 tbl_FC_sc_wt, tbl_FC_cd_wt, 
                 tbl_FC_sc_ts, tbl_FC_cd_ts, 
                 tbl_FC_sc_wt_dyrk, tbl_FC_cd_wt_dyrk, 
                 tbl_FC_sc_dyrk, tbl_FC_cd_dyrk)
...
cb_palette <- c(rep("#E69F00", 2), rep("#E69F00", 2),
                rep("#E69F00", 2),  rep("#999999", 2), 
                rep("#E69F00", 2), rep("#999999", 2), 
                rep("#E69F00", 2), rep("#999999", 2),
                rep("#E69F00", 2), rep("#999999", 2),
...
colors <- rep(cb_palette, 4)
unique(meanAll.byWeek_dev$groupPhase)
meanAll.byWeek_dev [meanAll.byWeek_dev$genotype == "trisomic SC",]
...
# unique(meanAll.byWeek_dev$genotype)
meanAll.byWeek_dev$genotype <- factor(meanAll.byWeek_dev$genotype, levels=c("wt control", "trisomic control", 
                                                                            "wt", "trisomic", 
                                                                            "wt_dyrk", "dyrk"))
tail(meanAll.byWeek_dev)
...
gAllByWeek_grid_simple <- gAllByWeek + 
    ...
    scale_shape_manual(values= rep(c(17, 15),10)) +
#     facet_grid(genotype ~ diet) + ylim(c(0,1))
    facet_grid(diet ~ genotype) +
    scale_y_continuous(breaks = c(0,0.25,0.5,0.75,1,1.25), limits = c(0, 1.4)) + 
    theme_update(strip.text.x = element_text (size=base_size * 1.3, face="bold")) +
    theme(plot.title = element_text(hjust = 0.5))
...
png(paste(home, "/2017_phecomp_marta/figures/", "circadian_day_night_trisomics_and_dyrk_by_genotype_simple_counts_zscore_no_filtered.png", sep=""), width=1400, height=400 )
# png(paste(home, "/2017_phecomp_marta/figures/", "circadian_day_night_trisomics_and_dyrk_by_genotype_simple_counts_zscore.png", sep=""), width=1400, height=400 )
...
########################
### tbl for statistics
tbl_all_stats <- rbind(tbl_control_wt, tbl_control_ts, tbl_FC_sc_wt, tbl_FC_cd_wt, tbl_FC_sc_ts, tbl_FC_cd_ts, 
                 tbl_FC_sc_wt_dyrk, tbl_FC_cd_wt_dyrk, tbl_FC_sc_dyrk, tbl_FC_cd_dyrk)
...
# write.foreign(as.data.frame(stats_mean_export), "/Users/jespinosa/sharedWin/2017_trisomics_spps_analysis/stats_mean_export.txt", "/Users/jespinosa/sharedWin/2017_trisomics_spps_analysis/stats_mean_export.sps",   package="SPSS") 
...
write.xlsx(postHoc_group_phase, 
           "/Users/jespinosa/2017_phecomp_marta/results/anova_group_phase/posthoc_bonferroni_sign.xlsx", 
           row.names =TRUE) 
