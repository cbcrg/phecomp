...
df.sigResults.Hab$significance
df.meanCase.m.Hab$foldChange
df.meanCase.m.Hab$stars <- cut(df.sigResults.Hab$significance, breaks=c(-Inf, 0.001, 0.01, 0.05, Inf), label=c("***", "**", "*", ""))

df.meanCase.m.Hab$variable <- ""
# heatMapPlotterHab (df.meanCase.m.Hab, main="\n",  weekNotation="N", legPos="none", xlab="\nHabituation Phase", ylab="Eating Behavior                          Drinking Behavior\n")
heatMapPlotterHab (df.meanCase.m.Hab, main="\n",  weekNotation="N", legPos="none", xlab="\n", ylab="\n", widthCol=10)

# ggsave (file=paste(home, "/dropboxTCoffee_new/Dropbox/jespinosa/2013phecomp2shareFinal/drafts_paper/figures_20151110/fig4/", "fig4_hab.tiff", sep=""), 
...
colnames (df.sigResults.SC) <- c("channel", "group", "period", "variable","significance")
df.sigResults.SC
#order the data by period
...
#fold change have to be numeric to make the function work
df.sigResults.SC$significance <- as.numeric (df.sigResults.SC$significance)
...
#########################
# Ordering the df.meanCase.SC.m.Dev and changing labels
...
head (df.meanCase.SC.m.Dev)
head (df.sigResults.SC.Dev)

df.meanCase.SC.m.Dev$stars <- cut(df.sigResults.SC.Dev$significance, breaks=c(-Inf, 0.001, 0.01, 0.05, Inf), label=c("***", "**", "*", ""))

df.meanCase.SC.m.Dev$variable <- ""
...
heatMapPlotter (df.meanCase.SC.m.Dev, main="\n",   weekNotation = "N", legPos="right", xlab="\n", ylab="\n")

# ggsave (file=paste(home, "/dropboxTCoffee_new/Dropbox/jespinosa/2013phecomp2shareFinal/drafts_paper/figures_20151110/fig4/", "fig4_B.tiff", sep=""), 
#         width=5.5, height=4.5, dpi=400)
...
colnames (df.sigResults.CD) <- c("channel", "group", "period", "variable","significance")
df.sigResults.CD
...
#significance have to be numeric to make the function work
df.sigResults.CD$significance <- as.numeric (df.sigResults.CD$significance)
...
head (df.sigResults.CD.Dev)
head (df.meanCase.CD.m.Dev)
df.meanCase.CD.m.Dev$significance <- df.sigResults.CD.Dev$significance 
  
df.meanCase.CD.m.Dev$stars <- cut(df.sigResults.CD.Dev$significance, breaks=c(-Inf, 0.001, 0.01, 0.05, Inf), label=c("***", "**", "*", ""))
df.meanCase.CD.m.Dev$variable <- ""

# heatMapPlotter (df.meanCase.CD.m.Dev, main="CM vs SC Channel (Significance Level)\n",   weekNotation = "N", legPos="right", xlab="\nDevelopment phase (weeks)", ylab="Eating Behavior                          Drinking Behavior\n")
heatMapPlotter (df.meanCase.CD.m.Dev, main="\n",   weekNotation = "N", legPos="right", xlab="\n", ylab="\n")

# ggsave (file=paste(home, "/dropboxTCoffee_new/Dropbox/jespinosa/2013phecomp2shareFinal/drafts_paper/figures_20151110/fig4/", "fig4_C.tiff", sep=""), 
#         width=5.5, height=4.5, dpi=400)
...
colnames (df.sigResults.CDvsSC) <- c("channel", "group", "period", "variable","significance")
...
#fold change have to be numeric to make the function work
df.sigResults.CDvsSC$significance <- as.numeric (df.sigResults.CDvsSC$significance)
#Volver a poner el campo como food_cd
...
head (df.sigResults.CDvsSC.Dev)
head (df.meanCase.CDvsSC.m.Dev)
df.meanCase.CDvsSC.m.Dev$significance <- df.sigResults.CDvsSC.Dev$significance
df.meanCase.CDvsSC.m.Dev$stars <- cut(df.sigResults.CDvsSC.Dev$significance, breaks=c(-Inf, 0.001, 0.01, 0.05, Inf), label=c("***", "**", "*", ""))
df.meanCase.CDvsSC.m.Dev$variable <- ""
# heatMapPlotter (df.meanCase.CDvsSC.m.Dev, main="CM vs SC channel\n",   weekNotation = "N", legPos="right", xlab="\nDevelopment phase (weeks)",ylab="Eating Behavior                          Drinking Behavior\n")
heatMapPlotter (df.meanCase.CDvsSC.m.Dev, main="\n",   weekNotation = "N", legPos="right", xlab="\n", ylab="\n")

ggsave (file=paste(home, "/dropboxTCoffee_new/Dropbox/jespinosa/2013phecomp2shareFinal/drafts_paper/figures_20151110/figS5/", "figS5.tiff", sep=""), 
        width=5.5, height=4.5, dpi=400)