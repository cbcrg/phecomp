...
data_reinst <- read.csv (paste (home, "/old_data/data/impulsivity_timeOutPresses.csv", sep=""), dec=",", sep=";")
head (data_reinst)
...
data_reinst_filt <- subset (data_reinst, select = -c(group_lab))
...
title_p <- paste ("PCA impulsivity - presses during time-out\n", sep="")
...
ggsave (p_circle_plot, , file=paste(home, "/old_data/figures/", 
                                                     "circle_",  "impulsivity_PCA", ".tiff", sep=""), width = 13, height = 13, dpi=dpi_q)
