...
dir_plots <- "/Dropbox (CRG)/2015_reinstatement_rafa/figures/lda_20160525/"
...
# cb_palette_adapt <- c("#999999", "#009E73", "#0072B2", "#E69F00", "#D55E00", "#CC79A7") #"#0072B2"
# filter_v <- dep_ad_lib
...
# EXTR<-subset (data_reinst_filt, select= c("group", "Ext_Inflex") )
# tapply(EXTR$Ext_Inflex, EXTR$group, summary)

length_tbl <- length(data_reinst_filt[1,]) -1

res_lda = lda(group  ~ . , data_reinst_filt)
# res_lda <- lda(as.matrix(data_reinst_filt[,1:length_tbl]), data_reinst_filt$group, scaling=TRUE) #same result

prop.lda = res_lda$svd^2/sum(res_lda$svd^2) * 100
plda <- predict(object = res_lda, newdata = data_reinst_filt)
# plda <- predict(object = res_lda, data_reinst_filt[,1:length_tbl]) # same thing does not change result

data_reinst_filt$group <- as.factor(data_reinst_filt$group)
...
#   scale_y_continuous (limits=c(-10,5), breaks=seq(-10,5, by=5)) +
  # whole_operant_cond
  scale_x_continuous (limits=c(-11,11), breaks=seq(-10,10, by=5)) + 
...
#   scale_y_continuous(limits=c(-3, 2), breaks=(seq(-3,2,2))) +
  # whole_operant_cond
#   scale_x_continuous(limits=c(-1, 1), breaks=(seq(-1,1,1))) +
#   scale_y_continuous(limits=c(-1, 1), breaks=(seq(-1,1,1))) +
#   # extinction
#   scale_x_continuous(limits=c(-2.2, 2.2), breaks=(seq(-2,2,1))) +
#   scale_y_continuous(limits=c(-2.2, 2.2), breaks=(seq(-2,2,1))) +
  # cue
  scale_x_continuous(limits=c(-1, 1), breaks=(seq(-1,1,1))) +
  scale_y_continuous(limits=c(-1, 1), breaks=(seq(-1,1,1))) +
...
#     scale_y_continuous(limits=c(-0, 0.5), breaks=(c(-1,0,1))) +
  ...
  geom_hline (yintercept=0, linetype="dotted") + #title_var_loadings
...
lda_reinstatement.pc1.pc2_leg_in <- lda_reinstatement.pc1.pc2_aspect_ratio_title +
...
panel_lda_reins <- ggdraw() + draw_plot (lda_reinstatement.pc1.pc2_leg_in, 0, .5, 0.5, .5) +
  ...
  draw_label(title_PCA_individuals, x = 0.25, y = 1, hjust=0.5, vjust = 1,
             size =  size_titles, fontface = 'bold') +
  draw_label(title_var_loadings, x = 0.75, y = 1, hjust=0.5, vjust = 1,
             size =  size_titles, fontface = 'bold')
# vjust = 1, hjust = 1,
...
img_format=".tiff"
dpi_q = 300
ggsave (panel_lda_reins, file=paste(home, dir_plots, "panel_LDA_reinst", tag_file, img_format, sep=""), 
