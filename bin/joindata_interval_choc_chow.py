...
def CheckArguments(arg):
  ...
  if(len(arg)<3):
    ...
    sys.stderr.write("\nPheCom Joining Data Files Tool.\nCreates a single file appending the content of all filenames provided.Differenciates between the different interval types (choc-choc, choc-chow, chow-choc, chow-chow.\n")
...
class Joiner:
  ...
  def JoinData(self): 
    ...
    if (self.opt=='k') :
      ...
      for key in self.FileInfo.keys():
        ...
        for i in range(index):
          counter.append(0)  # for calculating interval durations
          self.Old_disp_type.append('None')
        while (line.find("Stop Date")==-1):   # End of the data
          #j=0
          ...
          for i in range(len(info)/2):
            # Merge Data
            if (info[i]!='0.00' or info[i+6]!='0.00'):  # Mouse eat at one of the dispensers
              ...
              #print disp_type
              if (self.Old_disp_type[i]!='None'): 
                duration='%d'% counter[i]
                # Add a mark to know the eating event sequence
                ''' FROM 	Choc	TO	Choc	-> A
		    FROM 	Choc	TO	Chow	-> B
		    FROM 	Chow	TO	Choc	-> C
		    FROM 	Chow	TO	Chow	-> D
                ...
                '''
                if (self.Old_disp_type[i]=='Choc'):
                  if(disp_type=='Choc'):
                    self.TempHandle[i].write(duration+'A ')  # Store interval duration
                  else:
		    self.TempHandle[i].write(duration+'B ')  # Store interval duration
                else:
                  if(disp_type=='Choc'):
                    self.TempHandle[i].write(duration+'C ')  # Store interval duration
                  else:
		    self.TempHandle[i].write(duration+'D ')  # Store interval duration
              ...
              self.Old_disp_type[i]=disp_type
  ...
  def JoinTempFiles(self):
    ...
    try:
        self.handle_out1=open(self.FileInfo[0]['FileName'][:-4]+'_'+self.opt+'_merge_int.jnd', 'w') # Create/Open the file
    except:
    ...
    '''# Open Final Joined Data File
    try:
        self.handle_out2=open(self.FileInfo[0]['FileName'][:-4]+'_'+self.opt+'_merge_bin.jnd', 'w') # Create/Open the file
    except:
        sys.stderr.write("Unable create joined data file\n")
        raise ValueError()'''
    # Open Temporary Output Files
