...
''' HMM Trainer using PheCom Data. Returns new Hmm Model parameters saved in a .hmm file. Uses JAHMM library
    Usage:
...
from sys import *
import string
sys.path.append('/home/ifernan/jahmm/jahmm-0.6.1.jar')

from be.ac.ulg.montefiore.run.jahmm import ObservationInteger
from be.ac.ulg.montefiore.run.jahmm import OpdfInteger
...
class Trainer:
  ...
  def Symbols2Observations(self, symbol_list):
    arr = [ObservationInteger(self.ALPH.index(n)) for n in symbol_list]
    return arr
  ...
  def CreateDefaultHMM(self):
    ...
    # Create Default HMM
    self.hmm = Hmm(self.PI,self.A, self.E)
  ...
  def TrainModel(self):
    ...
    if (line!='' and line!='\n'):
      ...
      seq=line.split()
      train_seq= ArrayList()
      train_seq.add(self.Symbols2Observations(seq))
      #print line
      #print train_seq
      ...
      print 'train start' 
      self.hmm=learner.learn(self.hmm,train_seq)
      print 'train end'
      print self.hmm
      self.TrainModel()
    else:
...
def main():
  ...
  print sys.registry.getProperty('java.vm.vendor','???'),sys.registry.getProperty('java.vm.version','???')
