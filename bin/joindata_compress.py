...
def CheckArguments(arg):
  ...
  if(len(arg)<3):
    ...
    sys.stderr.write("\nPheCom Joining Data Files Tool.\nCreates a single file appending the content of all filenames provided. Data is in eating amount format (increments) and compression is used (eg. 10 non eating values , 0.00, stored as a single 0.00)\n")
...
class Joiner:
  ...
  def JoinTempFiles(self):
    ...
    try:
        self.handle_out=open(self.FileInfo[0]['FileName'][:-4]+'_'+self.opt+'_cpsd.jnd', 'w') # Create/Open the file
    except:
    ...
    for i in range(len(self.TempHandle)):
      ...
      info=line.split()  # Separate data by dispensers
      compress_counter=0 
      for value in info:
        if value=='0.00':
          compress_counter+=1  
          if compress_counter==10: 
             compress_counter=0  
             self.handle_out.write(value+' ')
        else:
          self.handle_out.write(value+' ')
