# Baum-Welch HMM trainer. Use of scaling variables
#
...
class BaumWelchTrainerHomogeneous:
  ...
  def GetRandomParam(self):
    ...
    # Initialize Counters
    for state in range(0,self.statesnbr+1): # for state in range(0,self.statesnbr+2):
      state='%d'%state
      self.a[state]={}
      self.e[state]={}
      for newstate in range(0,self.statesnbr+1): #for newstate in range(0,self.statesnbr+2):
        newstate='%d'%newstate
  ...
  def AssigRandomStates(self,seq):
    ...
    #print seq
    for interv in seq:
      '''if interv=='10' :# bin 10 goes directly to neutral state (last one)
        state=self.statesnbr+1
      else:'''
  ...
  def ManualParam(self):
    '''Initializes Model parameters Manually. Used for test purposes only '''
    ...
    self.a={'0': {'0': 0.0, '1': 0.5, '2': 0.5}, '1': {'0': 0.0, '1': 0.60, '2': 0.4}, '2': {'0': 0.0, '1': 0.4, '2': 0.6}}  
      ...
      for symbol in self.symbols.keys():
        ...
        for i in range(len(seq)):
          if seq[i]==symbol:
            ...
            print 'self.s[i]'
            print self.s[i]''' 
            summ+=self.f[state][i+1]*self.b[state][i]*self.s[i]# s[i+1]
            '''print 'summ'
            print summ'''
  ...
  def InitRoundLikelihood(self):
    ''' Inits likelihood to zero for next iteration '''
    self.log_prob_sum=0
  ...
  def LogLikelihoodEvol(self):
    ''' Prints log-likelihood evolution '''
