...
''' HMM Trainer using PheCom Data. Returns new Hmm Model parameters saved in a .hmm file
    Usage:
          hmmtrain [OPTION] params_file train_file
    where:
       'params_file' is a .par file having HMM default parammeters
       'train_file' is a .jnd file having train sequences for HMM Model
    OPTIONS:
	d	Deafault training.  Emission and transition probabilities trained
	t	Transition probabilities only training
'''   
...
import BaumWelchTrainer
from BaumWelchTrainer import *
...
def CheckArguments(arg):
  ...
  if(len(arg)<4):
    ...
    sys.stderr.write("d\tDefault training. Emission and transition probabilities trained\n") 
    sys.stderr.write("t\tTransition probabilities only training\n") 
...
class Trainer:
  ...
  def __init__(self,option,file_list):
    # Class Constructor
  ...
  def Train(self):
    ...
    trainer.BaumWelchInit(self.opt)
