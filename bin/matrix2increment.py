...
def CheckArguments(arg):
  ...
  if(len(arg)<3):
    ...
    sys.stderr.write("\nPheCom .mtx Intake Data Increment Converter.\nDetects differences between contigous acquisitions. Filters out eating amounts.\n")
    sys.stderr.write("Usage:\t python %s input_file\n" % arg[0])
    sys.stderr.write("where:\n'input_file' is a matrix .mtx file\n") 
    sys.stderr.write("OPTIONS:\n")
    sys.stderr.write("\ta\tConvert to increments all related files (SC and CD)\n") 
...
class Converter:
  ...
  def OpenFile(self,mtx_file):
    ...
    try:
      self.handle_out = open(mtx_file[:-4]+'_inc.mtx', 'w') # Create/Open the file
    except:
  ...
  def DataToIncrement(self): 
    ...
    while (line.find('Stop Date')==-1):  # End of file not reached
      ...
      for i in range(len(self.data)):
        if(self.data[i] < oldsample[i]):   # Eliminate positive changes --> mice put som meal in the hopper
          change=abs(oldsample[i]-self.data[i])   # Calculate the distance
          if change>=0.01: 
            if change<=0.8:  # Mouse has eaten/drunk a reasonable amount
              increment[i]='%.2f' % change
              #interval=0
            else:    # Greater values indicate a bigger peace has been taken out of the hopper, some eaten, some wasted. Value is fixed to meal mean: 0.06
              increment[i]='0.06'
