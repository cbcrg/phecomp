...
import BaumWelchTrainerHomogeneous
from BaumWelchTrainerHomogeneous import *
...
def CheckArguments(arg):
  ...
  if(len(arg)<3):
    ...
    sys.stderr.write("\nHMM Trainer using PheCom Data. Returns new Hmm Model parameters saved in a .hmm file. Training sequences are composed by bins with homogeneous contents.\n")
...
class Trainer:
  ...
  def OpenFileOut(self):
    ...
    try: 
      self.handle_out=open(self.datafile[:-4]+'_'+self.classes+'states.hmm','w')
    except: 
  ...
  def Train(self):
    ...
    while r < self.rounds:
      ...
      '''if r==0:
        trainer.ManualParam() # To BE DELETED!'''
      trainer.InitRoundLikelihood()
      train_seq=self.GetTrainSeq()
      #j=0
      #while j<2:
      while (len(train_seq)>1): # while (len(train_seq)>1 and threshold==False):
        #print 'training train seq:'
        #print train_seq 
        trainer.BaumWelchTrain(train_seq)
        train_seq=self.GetTrainSeq()
        #j+=1
      self.Hmm=trainer.CalculateParam()
      trainer.GetLogLikelihood()
      self.handle.close()
      r+=1
    trainer.LogLikelihoodEvol()
    trainer.PrintHMM()
    self.SaveHmmParam()
