...
''' PheCom Joining Data Files Tool. Creates a single file appending the content of all filenames provided. Transposes data in order to have a single row of data per each cage dispenser.
    It joins interval data files containing multiple entries per cage and without discarding any interval length. Returns:
    - Joined data file per cages
    - Corresponind bin file for the resulting joined file
    Usage:
...
def CheckArguments(arg):
  ...
  if(len(arg)<3):
    ...
    sys.stderr.write("\nPheCom Joining Data Files Tool.\nCreates a single file appending the content of all filenames provided.\n")
    sys.stderr.write("\nIt joins interval data files containing multiple entries per cage and without discarding any interval length. Returns:\n")
    sys.stderr.write("\n - Joined data file per cages.\n")
    sys.stderr.write("\n- Corresponind bin file for the resulting joined file.\n")
...
class Joiner:
  ...
  def CheckFilesTypes(self, file_list):  
    ...
    for filename in file_list[1:]:
      if (filename.find('SC')!=-1 or  filename.find('CD')!=-1):
        init=1
      elif(filename.find('FAT')!=-1 or  filename.find('CTRL')!=-1):
        init=4
      ...
      if (len(OldFileNamePrint)==len(FileNamePrint)):
        ...
        for i in range(init,len(FileNamePrint)):
  ...
  def JoinTempFiles(self):
    ...
    try:
        self.handle_out1=open(self.FileInfo[0]['FileName'][:-4]+'_'+self.opt+'_int.jnd', 'w') # Create/Open the file
    except:
